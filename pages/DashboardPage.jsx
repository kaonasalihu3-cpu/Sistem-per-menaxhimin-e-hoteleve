...
function DashboardPage() {
  ...
  const dashboardCards = useMemo(() => {
    ...
    return [
      ...
      { label: 'Total Reservations', value: summary.total_reservations },
      ...
      { label: 'Total Revenue', value: `$${Number(summary.total_income || 0).toFixed(2)}` },
      { label: 'Total Payments', value: summary.paid_invoices },
        ...
        <section className="mini-panel">
          ...
          <div className="mini-list">
            {recentPayments.map((item) => (
              <article className="mini-item" key={item.invoice_id}>
                <div>
                  <p>Invoice #{item.invoice_id}</p>
                  <p>{item.guest_name || 'Guest'} / Room {item.room_number || '-'}</p>
                </div>
                <div>
                  <p>${Number(item.amount || 0).toFixed(2)}</p>
                  <p>{String(item.paid_at).slice(0, 10)}</p>
                </div>
              </article>
            ))}
            {recentPayments.length === 0 ? <div className="state">No recent payments.</div> : null}
          </div>
        </section>
      </div>
